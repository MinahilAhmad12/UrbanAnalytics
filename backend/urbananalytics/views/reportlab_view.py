...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_reportlab_download_url(request, report_id):
    """
    Generate a pre-signed S3 download URL for a ReportLab-generated report.
    (Still works for reports already saved in DB)
    """
    try:
        ...
        file_key = report.file.replace(f"https://{settings.AWS_S3_CUSTOM_DOMAIN}/", "")
