...
# ------------------ Upload to S3 ------------------
def upload_report_to_s3(local_path, project_id, year, api_name="generate_yearly_report"):
...
# ---------------- PDF ----------------
def create_annual_report_pdf(instances, filename=None):
    """
    ...
    project_name = getattr(project, "name", None) or getattr(project, "project_name", None) or "N/A"
    ...
    section_title = ParagraphStyle("SectionTitle", parent=styles["Heading2"],
                                   textColor=colors.HexColor("#2E86C1"), fontSize=14, spaceAfter=6)
    normal = ParagraphStyle("NormalText", parent=styles["Normal"], fontSize=11, leading=16)
    footer_style = ParagraphStyle("Footer", parent=styles["Normal"], alignment=TA_RIGHT,
                                  fontSize=9, textColor=colors.grey)

    # -----------------------------
    # 1. Report Header
    # -----------------------------
    story.append(Paragraph("Urban Analytics – Annual Environmental Report", title_style))
    ...
    story.append(Paragraph(f"<b>Analysis Type:</b> {instance.analysis_type.upper()}", normal))
    story.append(Paragraph(f"<b>Year:</b> {instance.year}", normal))
    story.append(Paragraph(f"<b>Generated On:</b> {datetime.now().strftime('%d %b %Y %H:%M')}", normal))
    ...
    if hist_values:
        ...
        story.append(Paragraph("2.1 Overall Summary Statistics", section_title))
    ...
    try:
        if hist_values:
            ...
            story.append(Paragraph(f"3. {instance.analysis_type.upper()} Summary", section_title))
            ...
            colors_used = color_map.get(analysis_type_lower, ["#F4B400", "#0F9D58", "#DB4437"])

            card_data = [[
                Paragraph(f"<b>{instance.analysis_type.upper()} Avg</b><br/><font size=14><b>{avg_val}</b></font>",
                          ParagraphStyle("card", alignment=TA_CENTER, textColor=colors.white)),
                Paragraph(f"<b>{instance.analysis_type.upper()} Max</b><br/><font size=14><b>{max_val}</b></font>",
                          ParagraphStyle("card", alignment=TA_CENTER, textColor=colors.white)),
                Paragraph(f"<b>{instance.analysis_type.upper()} Min</b><br/><font size=14><b>{min_val}</b></font>",
                          ParagraphStyle("card", alignment=TA_CENTER, textColor=colors.white)),
            ]]
            ...
            if analysis_type_lower == "ndvi":
                categories = {"Bad": 0, "Moderate": 0, "Good": 0, "Very Good": 0, "Excellent": 0}
                ...
                categories = {"< 295K": 0, "295–300K": 0, "300–305K": 0, "305–310K": 0, "> 310K": 0}
                for val in hist_values:
                    if val < 295:
                        categories["< 295K"] += 1
                    elif val < 300:
            ...
            # Create bar chart
            dist_path = os.path.join(reports_dir, _unique_filename(f"dist_{analysis_type_lower}_{instance.year}", "png"))
            ...
            plt.title(chart_title, fontsize=11, fontweight="bold", pad=8, color="#2E86C1")
    ...
    try:
        if hist_values:
            line_path = os.path.join(
                reports_dir,
                _unique_filename(f"summary_{instance.analysis_type}_{instance.year}", "png")
            )
            ...
            plt.plot(metrics, values, marker="o", linewidth=2.5, color="#34495E", alpha=0.8)
            ...
            chart_section = KeepTogether([
                Paragraph("4. Summary Statistics Line Chart", section_title),
                Spacer(1, 6),
    ...
    # -----------------------------
    story.append(Paragraph("5. Interpretation", section_title))
    story.append(Paragraph(
    ...
    story.append(Paragraph("6. Recommendations", section_title))
    ...
    if analysis_type_lower == "ndvi":
        ...
        rec_text = (
            "Focus should be on improving low-performing zones based on this indicator’s environmental interpretation."
        )
    ...
    story.append(Spacer(1, 12))
     ...
     # -----------------------------
    # 8. Color Legend (Dynamic per Analysis Type)
    # -----------------------------
    story.append(Paragraph("7. Color Legend", section_title))
    ...
    for i, (color_code, _) in enumerate(legend_data):
        legend_table.setStyle([("BACKGROUND", (0, i), (0, i), colors.HexColor(color_code))])
...
def create_before_after_report_pdf(entries, filename=None):
    ...
    title_style = ParagraphStyle("TitleStyle", parent=styles["Heading1"], alignment=TA_CENTER, fontSize=20)
    section = ParagraphStyle("Section", parent=styles["Heading2"], textColor=colors.HexColor("#2E86C1"))
    normal = ParagraphStyle("Normal", parent=styles["Normal"], fontSize=11, leading=15)

    # -----------------------------
    # Header
    # -----------------------------
    story.append(Paragraph("Urban Analytics – Before-After Comparison Report", title_style))
    story.append(Spacer(1, 10))
    story.append(Paragraph(f"<b>Project:</b> {project_name}", normal))
    story.append(Paragraph(f"<b>Analysis Type:</b> {instance.analysis_type.upper()}", normal))
    story.append(Paragraph(f"<b>Compared Years:</b> {instance.before_year} → {instance.after_year}", normal))
    story.append(Paragraph(f"<b>Generated On:</b> {datetime.now().strftime('%d %b %Y %H:%M')}", normal))
    story.append(Spacer(1, 12))
    # -----------------------------
    # 1. Overview
    ...
    "reflect stress or declining conditions.",normal))
    story.append(Spacer(1, 12))
    ...
    try:
        ...
        stats_data = [
            ["Metric", f"Before ({instance.before_year})", f"After ({instance.after_year})"],
    ...
    try:
        if before_vals and after_vals:
            ...
            if atype == "ndvi":
                cards = [
                    ("NDVI Avg", safe_round(avg_before), safe_round(avg_after), "#F1C40F"),
                    ("NDVI Max", safe_round(max_before), safe_round(max_after), "#27AE60"),
                    ("NDVI Min", safe_round(min_before), safe_round(min_after), "#E74C3C"),
                ]
                section_title = "NDVI Summary"
                desc = "Higher NDVI indicates healthier vegetation and better environmental conditions."
            elif atype == "thermal":
                cards = [
                    ("Temp Avg", safe_round(avg_before), safe_round(avg_after), "#3498DB"),
                    ("Temp Max", safe_round(max_before), safe_round(max_after), "#E67E22"),
                    ("Temp Min", safe_round(min_before), safe_round(min_after), "#E74C3C"),
                ]
                section_title = "Thermal Summary"
                desc = "Higher temperature values indicate urban heat concentration or low vegetation areas."
            elif atype == "aqi":
                cards = [
                    ("AQI Avg", safe_round(avg_before), safe_round(avg_after), "#F1C40F"),
                    ("AQI Max", safe_round(max_before), safe_round(max_after), "#E74C3C"),
                    ("AQI Min", safe_round(min_before), safe_round(min_after), "#27AE60"),
                ]
                section_title = "Air Quality Summary"
                desc = "Lower AQI values represent better air quality; higher values indicate pollution."
            else:
                cards = [
                    ("Average", safe_round(avg_before), safe_round(avg_after), "#5DADE2"),
                    ("Maximum", safe_round(max_before), safe_round(max_after), "#2E86C1"),
                    ("Minimum", safe_round(min_before), safe_round(min_after), "#AED6F1"),
                ]
            ...
            for label, val_before, val_after, color in cards:
                arrow = "↑" if val_after > val_before else ("↓" if val_after < val_before else "→")
                arrow_color = "#000000"
                card_cells.append(
                    Paragraph(
                        f"<b>{label}</b><br/>"
                        ...
                        ParagraphStyle(name="CardText", alignment=TA_CENTER, textColor=colors.white, leading=14)
                    )
                )

            card_table = Table([card_cells], colWidths=[160] * len(cards), rowHeights=[55])
            for i, (_, _, _, color) in enumerate(cards):
                card_table.setStyle(TableStyle([("BACKGROUND", (i, 0), (i, 0), colors.HexColor(color))]))
            card_table.setStyle(TableStyle([
                ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                ("TEXTCOLOR", (0, 0), (-1, -1), colors.white),
                ("ALIGN", (0, 0), (-1, -1), "CENTER"),
            ...
            if atype == "ndvi":
                ...
                colors_palette_before = ["#F1948A", "#F8C471", "#F9E79F", "#ABEBC6", "#7DCEA0"]
                colors_palette_after = ["#E74C3C", "#F39C12", "#F1C40F", "#58D68D", "#27AE60"]
                ...
                colors_palette_before = ["#AED6F1", "#A9CCE3", "#F9E79F", "#EDBB99", "#F5B7B1"]
                colors_palette_after = ["#5DADE2", "#85C1E9", "#F4D03F", "#E67E22", "#E74C3C"]
                ...
                colors_palette_before = ["#ABEBC6", "#F9E79F", "#F5CBA7", "#F1948A", "#C39BD3"]
                colors_palette_after = ["#27AE60", "#F4D03F", "#E67E22", "#E74C3C", "#922B21"]
            ...
            for i in range(len(categories)):
                plt.bar(x[i] - bar_width / 2, before_perc[i], bar_width, color=colors_palette_before[i],
                        edgecolor="black", linewidth=0.3,
                        label=f"{instance.before_year}" if i == 0 else "")
                plt.bar(x[i] + bar_width / 2, after_perc[i], bar_width, color=colors_palette_after[i],
                        edgecolor="black", linewidth=0.3, alpha=0.9,
                        label=f"{instance.after_year}" if i == 0 else "")
                plt.text(x[i] - bar_width / 2, before_perc[i] + 1, f"{before_perc[i]:.1f}%", ha="center", fontsize=8)
                plt.text(x[i] + bar_width / 2, after_perc[i] + 1, f"{after_perc[i]:.1f}%", ha="center", fontsize=8)
            ...
            dist_chart_path = os.path.join(reports_dir, _unique_filename("before_after_distribution", "png"))
            plt.savefig(dist_chart_path, dpi=130)
            plt.close()

            # Group heading, chart, and caption together to prevent page split
            chart_block = []
            chart_block.append(Paragraph("2. Before–After Category Distribution", section))
    ...
    try:
        if before_vals and after_vals:
            ...
            if atype == "thermal":
                ...
                ax1.plot(years, stats["Min"], marker="o", color="#E74C3C", linewidth=2.2, label="Min")
                ax1.plot(years, stats["Max"], marker="o", color="#2ECC71", linewidth=2.2, label="Max")
                ax1.plot(years, stats["Avg"], marker="o", color="#3498DB", linewidth=2.5, label="Avg")

                # Plot heterogeneity on right axis
                ax2.plot(years, stats["Heterogeneity"], marker="o", color="#F1C40F", linewidth=2.2, label="Heterogeneity")
                ...
                for label, values, color in [
                    ...
                    for i, y in enumerate(values):
                        ax1.text(years[i], y + 0.2, f"{y:.2f}", color=color, ha="center", fontsize=8, fontweight="bold")

                for i, y in enumerate(stats["Heterogeneity"]):
                    ax2.text(years[i], y + 0.1, f"{y:.2f}", color="#F1C40F", ha="center", fontsize=8, fontweight="bold")

                # Labels and layout
                ax1.set_xlabel("Year", fontsize=9)
                ax1.set_ylabel("Temperature (Kelvin)", fontsize=9, color="#2E86C1")
                ax2.set_ylabel("Heterogeneity (Std. Dev.)", fontsize=9, color="#F1C40F")
                ax1.set_title(f"{instance.analysis_type.upper()} Summary Comparison", fontsize=11, color="#2E86C1", pad=10)
                ...
                ax1.legend(lines1 + lines2, labels1 + labels2, fontsize=8, loc="upper right", frameon=False)
                ...
                plt.plot(years, stats["Min"], marker="o", color="#E74C3C", linewidth=2.2, label="Min")
                plt.plot(years, stats["Max"], marker="o", color="#2ECC71", linewidth=2.2, label="Max")
                plt.plot(years, stats["Avg"], marker="o", color="#3498DB", linewidth=2.5, label="Avg")
                plt.plot(years, stats["Heterogeneity"], marker="o", color="#F1C40F", linewidth=2.2, label="Heterogeneity")

                # Annotate points
                for label, color in zip(stats.keys(), ["#E74C3C", "#2ECC71", "#3498DB", "#F1C40F"]):
                    vals = stats[label]
                ...
                plt.title(f"{instance.analysis_type.upper()} Summary Comparison", fontsize=11, color="#2E86C1", pad=10)
            ...
            line_chart_path = os.path.join(reports_dir, _unique_filename("before_after_linechart", "png"))
    ...
    Paragraph(
        ...
        ParagraphStyle('Trend', parent=normal, textColor=colors.HexColor('#2E86C1'))
    ),
    Spacer(1, 6),
]))
    ...
    story.append(Paragraph(
        f"Generated by Urban Analytics System • {datetime.now().strftime('%d %b %Y %H:%M')}",
        ParagraphStyle("Footer", alignment=TA_RIGHT, fontSize=9, textColor=colors.grey)
    ))
